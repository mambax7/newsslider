...
function nws_adminmenu($currentoption = 0, $breadcrumb = '')
{
    ...
    #buttonbar { float:left; width:100%; background: #DAE0D2 url('" . XOOPS_URL . "/modules/newsslider/assets/images/bg.gif') repeat-x left bottom; font-size: 12px; line-height:normal; border-left: 1px solid black; border-right: 1px solid black; margin-bottom: 12px; }

    #buttonbar ul { margin:0; margin-top: 15px; padding:10px 5px 0; list-style:none; }
        #buttonbar li { display:inline; margin:0; padding:0; }
        #buttonbar a { float:left; background:url('" . XOOPS_URL . "/modules/newsslider/assets/images/left_both.gif') no-repeat left top; margin:0; padding:0 0 0 9px; border-bottom:1px solid #000; text-decoration:none; white-space: nowrap}
        #buttonbar a span { float:left; display:block; background:url('" . XOOPS_URL . "/modules/newsslider/assets/images/right_both.gif') no-repeat right top; padding:5px 15px 4px 6px; font-weight:bold; color:#765; white-space: nowrap}
    ...
    if (file_exists(XOOPS_ROOT_PATH . '/modules/newsslider/language/' . $xoopsConfig['language'] . '/modinfo.php')) {
        require_once XOOPS_ROOT_PATH . '/modules/newsslider/language/' . $xoopsConfig['language'] . '/modinfo.php';
    } else {
    ...
    for ($i = 0, $iMax = count($adminObject); $i < $iMax; ++$i) {
